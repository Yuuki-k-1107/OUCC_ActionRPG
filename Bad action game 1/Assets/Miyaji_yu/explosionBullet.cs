...
public class explosionBullet : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag != "Player" && collision.gameObject.tag != "PlayerShot" && collision.gameObject.tag != "burst")
        {
            {
                zakoData.EnemHP -= 3;
                Debug.Log("Hit");
            }
            GameObject burst = Instantiate(burst_effect, this.transform.position, this.transform.rotation) as GameObject;
            Destroy(this.gameObject);
        }
    }
}